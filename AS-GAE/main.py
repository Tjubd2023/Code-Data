...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_flag', type=str, default='real_world')
    parser.add_argument('--data_dir', type=str, default='./data/real_world')
    parser.add_argument('--results_dir', type=str, default='./results')
    parser.add_argument('--real_world_name', type=str, default='cora')
    ...
    while flag:
        ...
        for epoch in range(1, 20):
            ...
            anomaly_nodes_index = anomaly_nodes_index.to('cuda:0')
            continuous_predicted_value = continuous_predicted_value.to('cuda:0')
            # print("residal_data.batch",residal_data.batch.device)
            # print("anomaly_scores",anomaly_scores.device)
            # print("anomaly_nodes_index",anomaly_nodes_index.device)
            # print("continuous_predicted_value",continuous_predicted_value.device)
        ...
        fpr, tpr, thresholds = roc_curve(anomaly_flag, continuous_predicted_value.cpu().numpy())
        predicted_label = []
        threshold = thresholds[-2]
        for i in range(len(continuous_predicted_value.cpu().numpy())):
            if continuous_predicted_value.cpu().numpy()[i] >= threshold:
                predicted_label.append(True)
            predicted_label.append(False)
